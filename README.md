...
**100% Bluenode** — 인하대학교 블록체인 학회 Bluenode(2023년 결성, 운영 3년차) 내부 멤버로 100% 구성된 팀의 KFIP 도전입니다. 외부 영입 없이 학회에서 이미 함께 활동해온 응집력을 보유합니다.

- **팀 구성**: Bluenode 학회 내부 멤버 100% (외부 영입 0명)
- **팀 역량**: 팀 내부에 XRPL EVM Sidechain 앱 개발 경험 보유
- **운영 trace**: BUIDL Asia 2026 참여, 국내 블록체인 학회·해커톤 활동 다수
- **포지셔닝**: 한국 XRPL 결제 표준화 grass-roots — 학회 internal 응집력 + 한국 XRPL 커뮤니티 코어 라인

**Why this team can ship**:
- 팀이 보유한 XRPL EVM Sidechain 앱 개발 경험 → v2.0 (6–18개월) XRPL EVM 합류는 실작동 코드 기반 단축 timeline
- 팀 전원이 학회에서 이미 함께 활동 → 응집력·iteration 속도 ↑
- **Bluenode 학회 internal 활용 가능성**: 학회 행사·굿즈·해커톤 상금 정산이 PayKit MVP의 internal beta 후보 use case. 팀-사용자 distance 짧음
